...
public class taruCom3JointStatePlayer : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        //    OnJointState);
        ros.Subscribe<JointStateMsg>(jointStatesTopicName, OnJointState);
    }

    void OnJointState(JointStateMsg msg)
    {
        Debug.Log($"[JointPlayer] OnJointState 呼ばれた。name数={msg.name.Length}, joints登録数={joints.Count}");
        if (emergencyStop && emergencyStop.isEmergencyStop)
            Debug.Log("[JointPlayer] emergencyStop で return");
            return;
        ...
        for (int i = 0; i < msg.name.Length; i++)
        {
            ...
            if (!joints.TryGetValue(msg.name[i], out var joint))
                Debug.Log($"[JointPlayer] '{msg.name[i]}' は joints に無い → スキップ");
